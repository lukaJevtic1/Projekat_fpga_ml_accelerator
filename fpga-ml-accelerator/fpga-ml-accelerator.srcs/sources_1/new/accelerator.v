...
module acclerator #(
    ...
    parameter N = 5,     //Bit width of activations and weights (total datapath width)
    parameter Q = 2,     //Number of fractional bits in case of fixed point representation
    parameter AW = 1,    //Needed in case of tanh activation function to set the size or ROM
    parameter DW = 5,    //Datapath width = N 
    ...
    pooler #(.N(N),.Q(Q),.m(n-k+1),.p(p),.ptype(ptype),.p_sqr_inv(psqr_inv)) pool( //Pooling Unit
            .clk(clk),
            .ce(valid_ip),
